...
if (isset($_POST['url'])) {
    ...
    if (file_exists($sDir.'/ps.txt')) {
        ...
        if (!file_exists($sDir.'/ressources.txt')) {
            ...
            fclose($fp);
            $tabRessources = ['js'=>[], 'css'=>[], 'images'=>[], 'imagesjpg'=>[], 'html'=>[]];
            ...
            fwrite($fp, '----Getting HTML : OK '.date('Y-m-d H:i:s')."\n");
            fclose($fp);
            ...
            if ($iNbTask == 0) {
                ...
                $files = ['index-nowebpack.html', 'log.txt', 'webpack.config.js'];
